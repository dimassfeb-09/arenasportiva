...
// jika court_id tidak dikirim, pakai lapangan pertama sesuai jenis
if (!isset($_GET['court_id']) && $jenis) {
...
// detail court & booking existing
$court_id = isset($_GET['court_id']) ? (int)$_GET['court_id'] : 0;
$bookings = $court_id ? getCourtBookings($mysqli, $court_id) : [];

// daftar semua lapangan untuk selector
$allCourts = [];
if ($jenis) {
    $stmt = $mysqli->prepare("SELECT id, name, type, price_per_hour 
                              FROM courts 
...
// setelah semua header selesai, include header.php
        ...
        <select id="courtSelect" name="court_id" class="form-select w-auto d-inline-block">
          <?php foreach ($allCourts as $c): ?>
            <option value="<?= $c['id'] ?>" <?= $c['id'] == $court_id ? 'selected' : '' ?>>
              ...
              (Rp <?= number_format($c['price_per_hour'], 0, ',', '.') ?>/jam)
            </option>
          <?php endforeach; ?>
        </select>
      </div>
          ...
          <div class="form-group">
            ...
            ?>
              <div class="form-check form-check-inline">
                <input id="<?= $inputId ?>" class="slot-input" type="radio" 
                       name="slot" value="<?= $slot ?>" <?= $disabled ?>>
...
<script>
  document.getElementById('courtSelect').addEventListener('change', function() {
    ...
    if (jenis) {
      ...
      window.location.href = "booking.php?court_id=" + courtId;
    }
  });
</script>
